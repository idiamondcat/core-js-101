...
function findFirstSingleChar(str) {
  const arr = str.split('');
  const uniqueArr = arr.find((elem, _, array) => array.indexOf(elem) === array.lastIndexOf(elem)) || '';
  return uniqueArr[0] || null;
}
...
function isCreditCardNumber(ccn) {
  ...
  return !/^\d+$/.test(str) || (str.split('').reduce((sum, d, n) => sum + parseInt(((n + str.length) % 2) ? d : [0, 2, 4, 6, 8, 1, 3, 5, 7, 9][d], 10), 0)) % 10 === 0;
}
...
function getMatrixProduct(m1, m2) {
  ...
  return m1.map((e, i) => m2[0].map((b, j) => e.reduce((a, c, d) => a + m1[i][d] * m2[d][j], 0)));
}
